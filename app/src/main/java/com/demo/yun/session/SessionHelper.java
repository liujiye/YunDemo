...
import com.demo.yun.session.action.RTSAction;
import com.demo.yun.session.action.SnapChatAction;
import com.demo.yun.session.action.TipAction;
import com.demo.yun.session.viewholder.MsgViewHolderAVChat;
...
//                        || message.getAttachment() instanceof RTSAttachment)
                ...
                else if (theApp.getCurAccount().getAccount().equals(message.getSessionId()))
                {
                    // 发给我的电脑 不允许撤回
                    return true;
                }
    ...
    private static List<PopupMenuItem> getMoreMenuItems(Context context, String sessionId, SessionTypeEnum sessionTypeEnum)
    {
        ...
        moreMenuItems.add(new PopupMenuItem(context, ACTION_HISTORY_QUERY, sessionId,
                sessionTypeEnum, theApp.CONTEXT.getString(R.string.message_history_query)));
        moreMenuItems.add(new PopupMenuItem(context, ACTION_SEARCH_MESSAGE, sessionId,
                sessionTypeEnum, theApp.CONTEXT.getString(R.string.message_search_title)));
        moreMenuItems.add(new PopupMenuItem(context, ACTION_CLEAR_MESSAGE, sessionId,
                sessionTypeEnum, theApp.CONTEXT.getString(R.string.message_clear)));
